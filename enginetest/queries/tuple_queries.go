...
func MakeTupleQueryTests(cb func(test QueryTest)) {
	testEquality := func(queryString string, left, right []interface{}, expected interface{}) {
		cb(QueryTest{
			Query:    queryString,
	...
	for _, test := range tupleEqualityTests {
		testEquality("SELECT (?, ?) = (?, ?)", test.left, test.right, test.isEqual)
		testEquality("SELECT (?, ?) IN ((?, ?))", test.left, test.right, test.isEqual)
		testEquality("SELECT (?, ?) != (?, ?)", test.left, test.right, not(test.isEqual))
		testEquality("SELECT (?, ?) NOT IN ((?, ?))", test.left, test.right, not(test.isEqual))
		if !test.skipSubqueryTests {
			testEquality("SELECT (?, ?) IN (SELECT ?, ?)", test.left, test.right, test.isEqual)
			testEquality("SELECT (?, ?) NOT IN (SELECT ?, ?)", test.left, test.right, test.isEqual)
		}
