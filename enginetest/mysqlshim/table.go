...
func (t Table) AddForeignKey(ctx *sql.Context, fk sql.ForeignKeyConstraint) error {
	...
	onDeleteStr := ""
	if fk.OnDelete != sql.ForeignKeyReferenceOption_DefaultAction {
		onDeleteStr = fmt.Sprintf(" ON DELETE %s", string(fk.OnDelete))
	}
	onUpdateStr := ""
	if fk.OnUpdate != sql.ForeignKeyReferenceOption_DefaultAction {
	...
	return t.db.shim.Exec(t.db.name, fmt.Sprintf("ALTER TABLE `%s`.`%s` ADD%s FOREIGN KEY (`%s`) REFERENCES `%s`.`%s` (`%s`)%s%s;",
		fk.Database, t.name, constraint, strings.Join(fk.Columns, "`,`"), fk.ReferencedDatabase, fk.ReferencedTable,
		strings.Join(fk.ReferencedColumns, "`,`"), onDeleteStr, onUpdateStr))
}
