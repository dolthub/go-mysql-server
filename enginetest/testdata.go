	...
	if includeTable(includedTables, "mytbl") {
		wrapInTransaction(t, myDb, harness, func() {
			table, err = harness.NewTable(myDb, "mytbl", sql.NewPrimaryKeySchema(sql.Schema{
				{Name: "i", Type: sql.Int64, Source: "mytbl", PrimaryKey: true},
				{Name: "j", Type: sql.Int64, Source: "mytbl"},
			}))
			if err == nil {
				InsertRows(t, NewContext(harness), mustInsertableTable(t, table),
...
func createNativeIndexes(t *testing.T, harness Harness, e *sqle.Engine) error {
	createIndexes := []string{
		...
		"create index mytbl_idx1 on mytbl (j)",
		"create index mytbl_idx2 on mytbl (j)",
	}
