...
func (j *joinOrderBuilder) ReorderJoin(n sql.Node) {
	j.m.Tracer.Log("Starting join reordering for node: %T", []interface{}{n}...)
	j.populateSubgraph(n)

	if j.useFastReorder() {
		j.m.Tracer.Log("Using fast reorder algorithm (large join with %d tables)", []interface{}{len(j.vertices)}...)
		if j.buildSingleLookupPlan() {
			j.m.Tracer.Log("Successfully built single lookup plan", []interface{}{}...)
			return
		}
		j.m.Tracer.Log("Failed to build single lookup plan, falling back to exhaustive enumeration", []interface{}{}...)
	} else if j.hasCrossJoin {
		j.m.Tracer.Log("Join contains cross joins, attempting single lookup plan first", []interface{}{}...)
		// Rely on FastReorder to avoid plans that drop filters with cross joins
		if j.buildSingleLookupPlan() {
			j.m.Tracer.Log("Successfully built single lookup plan for cross join", []interface{}{}...)
			return
		}
		j.m.Tracer.Log("Failed to build single lookup plan for cross join, using exhaustive enumeration", []interface{}{}...)
	}
	...
	j.m.Tracer.Log("Building transitive closure for %d edges", []interface{}{len(j.edges)}...)
	j.ensureClosure(j.m.root)
	j.m.Tracer.Log("Starting exhaustive subset enumeration", []interface{}{}...)
	j.dpEnumerateSubsets()
	j.m.Tracer.Log("Completed join reordering", []interface{}{}...)
	return
}
...
func (j *joinOrderBuilder) buildJoinOp(n *plan.JoinNode) *ExprGroup {
	j.m.Tracer.Log("Building join operation: %s", []interface{}{n.JoinType().String()}...)
	leftV, leftE, _ := j.populateSubgraph(n.Left())
	rightV, rightE, _ := j.populateSubgraph(n.Right())
	typ := n.JoinType()
	if typ.IsPhysical() {
		typ = plan.JoinTypeInner
		j.m.Tracer.Log("Converted physical join type to inner join", []interface{}{}...)
	}
	isInner := typ.IsInner()
	op := &operator{
		joinType:      typ,
	...
	j.m.Tracer.Log("Join filters: %v", []interface{}{filters}...)
	...
	if !ok {
		...
		j.plans[union] = group
		j.m.root = group
		j.m.Tracer.Log("Created new memo group for join combination", []interface{}{}...)
	}

	if !isInner {
		j.m.Tracer.Log("Building non-inner edge for join type: %s", []interface{}{typ.String()}...)
		j.buildNonInnerEdge(op, filters...)
	} else {
		j.m.Tracer.Log("Building inner edge for join type: %s", []interface{}{typ.String()}...)
...
func (j *joinOrderBuilder) addPlans(s1, s2 vertexSet) {
	j.m.Tracer.Log("Considering join between vertex sets s1=%s, s2=%s", []interface{}{s1.String(), s2.String()}...)
	...
	if j.plans[s1] == nil || j.plans[s2] == nil {
		...
		j.m.Tracer.Log("Skipping join - one or both input plans are nil", []interface{}{}...)
		return
	}
	...
	for i, ok := j.innerEdges.Next(0); ok; i, ok = j.innerEdges.Next(i + 1) {
		...
		if e.applicable(s1, s2) {
			...
			addInnerJoin = true
			j.m.Tracer.Log("Found applicable inner edge %d with filters: %v", []interface{}{i, e.filters}...)
		}
	}
	...
	for i, ok := j.nonInnerEdges.Next(0); ok; i, ok = j.nonInnerEdges.Next(i + 1) {
		e := &j.edges[i]
		if e.applicable(s1, s2) {
			j.m.Tracer.Log("Found applicable non-inner edge %d, adding join: %s", []interface{}{i, e.op.joinType.String()}...)
		...
		if e.applicable(s2, s1) {
			...
			// in DPSube()
			j.m.Tracer.Log("Found applicable non-inner edge %d (swapped), adding join: %s", []interface{}{i, e.op.joinType.String()}...)
	...
	if addInnerJoin {
		...
		if innerJoinFilters == nil {
			j.m.Tracer.Log("Adding cross join between s1=%s, s2=%s", []interface{}{s1.String(), s2.String()}...)
		...
		j.m.Tracer.Log("No applicable edges found for join between s1=%s, s2=%s", []interface{}{s1.String(), s2.String()}...)
	}
}
