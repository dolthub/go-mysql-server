...
func (ranges MySQLRangeCollection) String(ctx *Context) string {
	sb := strings.Builder{}
	sb.WriteByte('[')
	for i, rang := range ranges {
		...
		sb.WriteString(rang.String(ctx))
	}
	sb.WriteByte(']')
	return sb.String()
}
...
func (rang MySQLRange) ExpressionByColumnName(ctx *Context, idx Index, colExpr string) (MySQLRangeColumnExpr, bool) {
	for i, idxColExpr := range idx.Expressions(ctx) {
...
func (rang MySQLRange) String(ctx *Context) string {
	sb := strings.Builder{}
	sb.WriteByte('{')
	...
	for i := 0; i < len(rangeColl)-1; i++ {
		for j := i + 1; j < len(rangeColl); j++ {
			if ok, err := rangeColl[i].Overlaps(ctx, rangeColl[j]); err != nil {
				...
				return fmt.Errorf("overlapping ranges: %s and %s", rangeColl[i].String(ctx), rangeColl[j].String(ctx))
			}
		}
	}
	return nil
