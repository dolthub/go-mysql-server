...
//
// WindowPartitionIter returns rows in the same order generated by [child].
// materializeInput records each row's original position before sorting, and
// materializeOutput places results back at those positions and removes holes.
//
...
// materializeOutput evaluates aggregation results and places them at their original row positions.
// Empty positions are removed for grouped windows that produce fewer results than input rows.
func (i *WindowPartitionIter) materializeOutput(ctx *sql.Context) (sql.WindowBuffer, error) {
	if len(i.input) == 0 {
		return nil, io.EOF
	}

	output := make(sql.WindowBuffer, len(i.input))
	var row sql.Row
	var err error
	for {
		...
		originalOrderIdx := len(row) - 1
		output[row[originalOrderIdx].(int)] = row[:originalOrderIdx]
	}

	return slices.DeleteFunc(output, func(row sql.Row) bool { return row == nil }), nil
}
