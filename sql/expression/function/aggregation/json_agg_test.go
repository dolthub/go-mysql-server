...
func TestJsonArrayAgg_SimpleIntField(t *testing.T) {
	...
	j.Update(ctx, b, sql.NewRow(int32(7)))
	j.Update(ctx, b, sql.NewRow(int32(2)))

	v, err := j.Eval(ctx, b)
	assert.NoError(err)
	assert.Equal([]interface{}{int32(7), int32(2)}, v)
}
...
func TestJsonArrayAgg_Strings(t *testing.T) {
	...
	v, err := j.Eval(ctx, b)
	assert.NoError(err)
	assert.Equal([]interface{}{"hi","hello"}, v)
}
...
func TestJsonArrayAgg_JSON(t *testing.T) {
	...
	b := j.NewBuffer()
	j.Update(ctx, b, sql.NewRow(map[string]interface{}{"key1": "value1", "key2": "value2"}))

	v, err := j.Eval(ctx, b)
	assert.NoError(err)
	assert.Equal([]interface{}{map[string]interface{}{"key1": "value1", "key2": "value2"}}, v)
}
