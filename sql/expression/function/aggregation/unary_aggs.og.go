...
//
// TODO: optgen to regenerate this file isn't safe right now: its WithChildren template
// produces a signature missing the *sql.Context parameter, which is stale relative to sql.Expression
// Sum.Type and Avg.Type below are hand-patched to match unary_aggs.yaml's retType: types.Float64
// (Sum/Avg always accumulate as float64 internally, regardless of the child expression's type).
...
func (a *Avg) Type(ctx *sql.Context) sql.Type {
	// TODO: Why did we default this to float64 in aggregates.go?
