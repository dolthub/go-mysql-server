...
func (l *Lag) String(ctx *sql.Context) string {
	sb := strings.Builder{}
	if len(l.ChildExpressions) > 1 {
		sb.WriteString(fmt.Sprintf("lag(%s, %d, %s)", l.ChildExpressions[0].String(ctx), l.offset, l.ChildExpressions[1].String(ctx)))
	} else {
		sb.WriteString(fmt.Sprintf("lag(%s, %d)", l.ChildExpressions[0].String(ctx), l.offset))
	}
	if l.window != nil {
		sb.WriteString(" ")
...
func (l *Lag) DebugString(ctx *sql.Context) string {
	sb := strings.Builder{}
	if len(l.ChildExpressions) > 1 {
		sb.WriteString(fmt.Sprintf("lag(%s, %d, %s)", l.ChildExpressions[0].String(ctx), l.offset, l.ChildExpressions[1]))
	} else {
		sb.WriteString(fmt.Sprintf("lag(%s, %d)", l.ChildExpressions[0].String(ctx), l.offset))
	}
	if l.window != nil {
