...
func TestRemove(t *testing.T) {
	...
	testCases := []struct {
		...
		{f1, sql.Row{json, "$.[0]"}, nil, fmt.Errorf("Invalid JSON path expression. Expected field name after '.' at character 2 of $.[0]")},           // improper struct indexing
		...
		{f1, sql.Row{1, "$"}, nil, sql.ErrInvalidJSONArgument.New(1, "json_remove")},
		{f1, sql.Row{"}{", "$"}, nil, sql.ErrInvalidJSONText.New(1, "json_remove", "}{")},
