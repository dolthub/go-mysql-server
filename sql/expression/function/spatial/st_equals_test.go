...
func TestSTEquals(t *testing.T) {
	...
	t.Run("point vs point equals", func(t *testing.T) {
		...
		f := NewSTEquals(ctx, expression.NewLiteral(p1, types.PointType{}), expression.NewLiteral(p2, types.PointType{}))
	...
	t.Run("null vs point is null", func(t *testing.T) {
		...
		f := NewSTEquals(ctx, expression.NewLiteral(nil, types.Null), expression.NewLiteral(p, types.PointType{}))
		v, err := f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(nil, v)
	})
...
func TestSTEqualsSkipped(t *testing.T) {
	...
	t.Run("linestring vs linestring equals", func(t *testing.T) {
		...
		f := NewSTEquals(ctx, expression.NewLiteral(l1, types.LineStringType{}), expression.NewLiteral(l2, types.LineStringType{}))
		v, err := f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(true, v)
	})
	...
	t.Run("polygon vs multilinestring not equal", func(t *testing.T) {
		...
		f := NewSTEquals(ctx, expression.NewLiteral(p, types.PolygonType{}), expression.NewLiteral(ml, types.MultiLineStringType{}))
		v, err := f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(false, v)
	})
	...
	t.Run("empty geometry vs empty geometry equal", func(t *testing.T) {
		...
		g1 := types.GeomColl{}
		g2 := types.GeomColl{}
		f := NewSTEquals(ctx, expression.NewLiteral(g1, types.GeomCollType{}), expression.NewLiteral(g2, types.GeomCollType{}))
		v, err := f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(true, v)
	})
