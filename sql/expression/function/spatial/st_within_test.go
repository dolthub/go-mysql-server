...
func TestPointWithinLineString(t *testing.T) {
	...
	t.Run("terminal points are not within linestring", func(t *testing.T) {
		require := require.New(t)
		f := NewWithin(expression.NewLiteral(simpleLineString.Points[0], sql.PointType{}), expression.NewLiteral(simpleLineString, sql.PointType{}))
		v, err := f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(false, v)

		f = NewWithin(expression.NewLiteral(simpleLineString.Points[2], sql.PointType{}), expression.NewLiteral(simpleLineString, sql.PointType{}))
		v, err = f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(false, v)
	})
...
func TestPointWithinPolygon(t *testing.T) {
	t.Run("point within polygon", func(t *testing.T) {
		require := require.New(t)
		f := NewWithin(expression.NewLiteral(sql.Point{}, sql.PointType{}), expression.NewLiteral(square, sql.PolygonType{}))
		v, err := f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(true, v)
	})
	...
	t.Run("point within polygon (square) with hole", func(t *testing.T) {
		...
		f := NewWithin(expression.NewLiteral(p1, sql.PointType{}), expression.NewLiteral(squareWithHole, sql.PolygonType{}))
		v, err := f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(true, v)
		...
		p2 := sql.Point{X: -3, Y: 0}
		f = NewWithin(expression.NewLiteral(p2, sql.PointType{}), expression.NewLiteral(squareWithHole, sql.PolygonType{}))
		v, err = f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(true, v)
		...
		f = NewWithin(expression.NewLiteral(p3, sql.PointType{}), expression.NewLiteral(squareWithHole, sql.PolygonType{}))
		v, err = f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(true, v)
	})
	...
	t.Run("point within polygon (diamond) with hole", func(t *testing.T) {
		...
		f := NewWithin(expression.NewLiteral(p1, sql.PointType{}), expression.NewLiteral(diamondWithHole, sql.PolygonType{}))
		v, err := f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(true, v)
		...
		f = NewWithin(expression.NewLiteral(p2, sql.PointType{}), expression.NewLiteral(diamondWithHole, sql.PolygonType{}))
		v, err = f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(true, v)

		p3 := sql.Point{X: -1, Y: -2}
		f = NewWithin(expression.NewLiteral(p3, sql.PointType{}), expression.NewLiteral(diamondWithHole, sql.PolygonType{}))
		v, err = f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(true, v)
	})
	...
	t.Run("point on polygon boundary not within", func(t *testing.T) {
		...
		f := NewWithin(expression.NewLiteral(diamond.Lines[0].Points[0], sql.PointType{}), expression.NewLiteral(diamond, sql.PolygonType{}))
		v, err := f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(false, v)

		f = NewWithin(expression.NewLiteral(diamond.Lines[0].Points[1], sql.PointType{}), expression.NewLiteral(diamond, sql.PolygonType{}))
		v, err = f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(false, v)

		f = NewWithin(expression.NewLiteral(diamond.Lines[0].Points[2], sql.PointType{}), expression.NewLiteral(diamond, sql.PolygonType{}))
		v, err = f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(false, v)

		f = NewWithin(expression.NewLiteral(diamond.Lines[0].Points[3], sql.PointType{}), expression.NewLiteral(diamond, sql.PolygonType{}))
		v, err = f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(false, v)
	})
	...
	t.Run("point not within polygon intersects vertex", func(t *testing.T) {
		...
		p1 := sql.Point{X: -1, Y: 4}
		f := NewWithin(expression.NewLiteral(p1, sql.PointType{}), expression.NewLiteral(diamond, sql.PolygonType{}))
		v, err := f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(false, v)

		// passes through vertex a and c
		p2 := sql.Point{X: -5, Y: 0}
		f = NewWithin(expression.NewLiteral(p2, sql.PointType{}), expression.NewLiteral(diamond, sql.PolygonType{}))
		v, err = f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(false, v)

		// passes through vertex d
		p3 := sql.Point{X: -1, Y: -4}
		f = NewWithin(expression.NewLiteral(p3, sql.PointType{}), expression.NewLiteral(diamond, sql.PolygonType{}))
		v, err = f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(false, v)
	})
...
func TestPointWithinMultiPoint(t *testing.T) {
	t.Run("points within multipoint", func(t *testing.T) {
		...
		f := NewWithin(expression.NewLiteral(simpleMultiPoint.Points[0], sql.PointType{}), expression.NewLiteral(simpleMultiPoint, sql.MultiPointType{}))
		v, err := f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(true, v)
	...
	t.Run("point not within multipoint", func(t *testing.T) {
		...
		f := NewWithin(expression.NewLiteral(sql.Point{}, sql.PointType{}), expression.NewLiteral(simpleMultiPoint, sql.MultiPointType{}))
		v, err := f.Eval(sql.NewEmptyContext(), nil)
		require.NoError(err)
		require.Equal(false, v)
	})
