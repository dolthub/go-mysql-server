...
func TestConcatWithSeparator(t *testing.T) {
	t.Run("multiple arguments", func(t *testing.T) {
		...
		f, err := NewConcatWithSeparator(sql.NewEmptyContext(), expression.NewLiteral(",", types.LongText),
	...
	t.Run("separator is nil", func(t *testing.T) {
		require := require.New(t)
		f, err := NewConcatWithSeparator(sql.NewEmptyContext(), expression.NewLiteral(nil, types.LongText),
			expression.NewLiteral("foo", types.LongText),
			expression.NewLiteral(5, types.LongText),
			expression.NewLiteral(true, types.Boolean),
		)
...
func TestNewConcatWithSeparator(t *testing.T) {
	...
	_, err := NewConcatWithSeparator(sql.NewEmptyContext(), expression.NewLiteral(nil, types.LongText))
	require.NoError(err)

	_, err = NewConcatWithSeparator(sql.NewEmptyContext(), expression.NewLiteral(nil, types.LongText), expression.NewLiteral(nil, types.Int64))
	require.NoError(err)
