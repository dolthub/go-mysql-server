...
type Column struct {
	...
	// OnUpdate specifies automatic timestamp update behavior via [*sqlparser.OnUpdateExpr].
	OnUpdate *sqlparser.OnUpdateExpr
...
func FormatColumnExtra(c *Column) string {
	...
	if c.OnUpdate != nil {
		parts = append(parts, "on update "+sqlparser.String(c.OnUpdate))
	}
