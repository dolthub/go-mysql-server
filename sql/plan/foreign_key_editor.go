			...
			err := func() error {
				...
				if _, err = rowIter.Next(ctx); err == nil {
					return sql.ErrForeignKeyParentViolation.New(refActionData.ForeignKey.Name,
						refActionData.ForeignKey.Table, refActionData.ForeignKey.ReferencedTable, refActionData.RowMapper.GetKeyString(old))
				}
				return nil
			}()
			if err != nil {
...
func (fkEditor *ForeignKeyEditor) Delete(ctx *sql.Context, row sql.Row) error {
	//TODO: may need to process some cascades after the update to avoid recursive violations
	for _, refActionData := range fkEditor.RefActions {
		...
		default: // RESTRICT and friends
			err := func() error {
				...
				if _, err = rowIter.Next(ctx); err == nil {
					return sql.ErrForeignKeyParentViolation.New(refActionData.ForeignKey.Name,
						refActionData.ForeignKey.Table, refActionData.ForeignKey.ReferencedTable, refActionData.RowMapper.GetKeyString(row))
				}
				return nil
			}()
			if err != nil {
...
func (reference *ForeignKeyReferenceHandler) CheckReference(ctx *sql.Context, row sql.Row) error {
	...
	if reference.ForeignKey.IsSelfReferential() {
		...
		for i := range reference.ForeignKey.Columns {
			...
			refPos := reference.SelfCols[reference.ForeignKey.ReferencedColumns[i]]
	...
	return sql.ErrForeignKeyChildViolation.New(reference.ForeignKey.Name, reference.ForeignKey.Table,
		reference.ForeignKey.ReferencedTable, reference.RowMapper.GetKeyString(row))
}
...
// GetChildParentMapping returns a mapping from the foreign key columns of a child schema to the parent schema. The position
...
func GetChildParentMapping(parentSch sql.Schema, childSch sql.Schema, fkDef sql.ForeignKeyConstraint) ([]int, error) {
	...
	for i := range fkDef.Columns {
		...
		parentIndex, ok := parentMap[strings.ToLower(fkDef.ReferencedColumns[i])]
		if !ok {
			return nil, fmt.Errorf("foreign key `%s` refers to column `%s` on referenced table `%s` but it could not be found",
				fkDef.Name, fkDef.ReferencedColumns[i], fkDef.ReferencedTable)
		}
