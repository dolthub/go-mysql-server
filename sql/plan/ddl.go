...
// WithReferencedForeignKeyTables adds the tables that are referenced in each foreign key. The table indices is assumed
// to match the foreign key indices in their respective slices.
func (c *CreateTable) WithReferencedForeignKeyTables(refTbls []sql.ForeignKeyTable) (*CreateTable, error) {
...
func (c *CreateTable) createForeignKeys(ctx *sql.Context, tableNode sql.Table) error {
	...
	for i, fkDef := range c.fkDefs {
		...
		if fkChecks.(int8) == 1 {
			...
			err = resolveForeignKey(ctx, fkTbl, fkRefTbl, fkDef)
			if err != nil {
				return err
			}
		}
