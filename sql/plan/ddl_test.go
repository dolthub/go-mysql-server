...
func TestDropTable(t *testing.T) {
	...
	d := NewDropTable([]sql.Node{NewResolvedTable(memory.NewTable("testTable1", s), db, nil), NewResolvedTable(memory.NewTable("testTable2", s), db, nil)}, false)
	...
	d = NewDropTable([]sql.Node{NewResolvedTable(memory.NewTable("testTable1", s), db, nil)}, false)
	_, err = d.RowIter(sql.NewEmptyContext(), nil)
	require.Error(err)

	d = NewDropTable([]sql.Node{NewResolvedTable(memory.NewTable("testTable3", s), db, nil)}, false)
	_, err = d.RowIter(sql.NewEmptyContext(), nil)
	require.NoError(err)
