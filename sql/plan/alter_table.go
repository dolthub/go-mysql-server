...
// ColumnReferencedInFunctionalIndex returns true if the given column name is referenced
// in the generated expression of a hidden system column (i.e., a functional index dependency).
// Handles both resolved expression trees and unresolved raw-string expressions (e.g. from Dolt storage).
func ColumnReferencedInFunctionalIndex(generated *sql.ColumnDefaultValue, colName string) bool {
...
func (d *DropColumn) Validate(ctx *sql.Context, tbl sql.Table) error {
	...
	for _, col := range d.targetSchema {
		if col.HiddenSystem && col.Generated != nil && ColumnReferencedInFunctionalIndex(col.Generated, colLower) {
