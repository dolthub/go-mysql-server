...
func (r *RenameColumn) RowIter(ctx *sql.Context, row sql.Row) (sql.RowIter, error) {
	...
	if fkTable, ok := alterable.(sql.ForeignKeyTable); ok {
		...
		if len(parentFks) > 0 || len(fks) > 0 {
			if resolvedTable, ok := r.Child.(*ResolvedTable); ok {
				err = handleColumnRename(ctx, fkTable, resolvedTable.Database, r.ColumnName, r.NewColumnName)
				if err != nil {
					return nil, err
				}
			} else {
				return nil, fmt.Errorf("updating a column referenced by a foreign key requires a visible database")
			}
		}
	}
...
func (m *ModifyColumn) RowIter(ctx *sql.Context, row sql.Row) (sql.RowIter, error) {
	...
	if fkTable, ok := alterable.(sql.ForeignKeyTable); ok {
		...
		if len(parentFks) > 0 || len(fks) > 0 {
			if resolvedTable, ok := m.Child.(*ResolvedTable); ok {
				...
				err = handleColumnRename(ctx, fkTable, resolvedTable.Database, m.columnName, m.column.Name)
				if err != nil {
					return nil, err
				}
			} else {
...
func handleColumnRename(ctx *sql.Context, fkTable sql.ForeignKeyTable, db sql.Database, oldName string, newName string) error {
	...
	if len(parentFks) > 0 {
		...
		for _, parentFk := range parentFks {
			...
			if shouldUpdate {
				parentTable, ok, err := db.GetTableInsensitive(ctx, parentFk.ParentTable)
				if err != nil {
					return err
				}
				if !ok {
					return sql.ErrTableNotFound.New(parentFk.ParentTable)
				}
				err = parentTable.(sql.ForeignKeyTable).UpdateForeignKey(ctx, parentFk.Name, parentFk)
				if err != nil {
					return err
				}
			}
