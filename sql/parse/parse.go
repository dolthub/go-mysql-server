...
func convertAlterTableClause(ctx *sql.Context, query string, ddl *sqlparser.DDL) ([]sql.Node, error) {
	...
	// if there were no nodes generated by the above logic, fall back on the normal DDL logic, which handles some ALTER 
	// TABLE statements like rename
	if len(ddlNodes) == 0 {
		// TODO: what is this bool for
		ddlNode, err := convertDDL(ctx, query, ddl, true)
		if err != nil {
			return nil, err
		}

		ddlNodes = append(ddlNodes, ddlNode)
	}
