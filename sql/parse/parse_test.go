...
END`,
		...
		{
			input: "CREATE DEFINER = `user1`@`localhost` EVENT IF NOT EXISTS mydb.event1 ON SCHEDULE AT CURRENT_TIMESTAMP + INTERVAL '2:3' MONTH_DAY ON COMPLETION NOT PRESERVE ENABLE DO SELECT 1",
			plan: plan.NewCreateEvent(
				sql.UnresolvedDatabase("mydb"),
				"event1",
				"`user1`@`localhost`",
				plan.NewOnScheduleTimestamp(
					...
					[]sql.Expression{expression.NewInterval(expression.NewLiteral("2:3", types.LongText), "MONTH_DAY")},
				),
				nil,
				nil,
				nil,
