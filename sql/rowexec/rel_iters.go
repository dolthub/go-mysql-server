...
func validateSystemVariableValue(ctx *sql.Context, sysVarName string, val any) error {
	...
	case "sql_mode":
		...
		switch v := val.(type) {
		case uint64:
			if v&sql.MODE_NO_ZERO_IN_DATE == 0 {
			...
			if (hasStrict || hasErrorForDivisionByZero || hasNoZeroDate) &&
				!(hasStrict && hasErrorForDivisionByZero && hasNoZeroDate) {
		...
		case string:
			v = strings.ToUpper(v)
			if !strings.Contains(v, sql.NO_ZERO_IN_DATE) {
				return sql.ErrMissingNoZeroInDateSQLMode.New()
			}
			// If it contains any one of these "strict" sql_modes,
			// then warn if it does not contain all the "strict" modes
