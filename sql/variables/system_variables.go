...
var systemVars = map[string]sql.SystemVariable{
	...
	"sql_mode": &sql.MysqlSystemVariable{
		...
		Type: types.NewSystemSetType(
			...
			sql.NO_UNSIGNED_SUBTRACTION,
			sql.NO_DIR_IN_CREATE,
			...
			sql.STRICT_ALL_TABLES,
			sql.NO_ZERO_IN_DATE,
			sql.NO_ZERO_DATE,
			sql.ALLOW_INVALID_DATES,
			sql.ERROR_FOR_DIVISION_BY_ZERO,
			sql.TRADITIONAL,
			sql.NOT_USED_29,
			sql.HIGH_NOT_PRECEDENCE,
			sql.NO_ENGINE_SUBSTITUTION,
			sql.PAD_CHAR_TO_FULL_LENGTH,
			sql.TIME_TRUNCATE_FRACTIONAL,
			sql.INTERPRET_UTF8_AS_UTF8MB4,
		),
