...
func Parse(ctx *sql.Context, cat sql.Catalog, authHandler AuthorizationHandler, query string) (sql.Node, *sql.QueryFlags, error) {
	return ParseWithOptions(ctx, cat, authHandler, query, sql.LoadSqlMode(ctx).ParserOptions())
}

func ParseWithOptions(ctx *sql.Context, cat sql.Catalog, authHandler AuthorizationHandler, query string, options ast.ParserOptions) (sql.Node, *sql.QueryFlags, error) {
	// TODO: need correct parser
	b := New(ctx, cat, sql.NewMysqlParser(), authHandler)
