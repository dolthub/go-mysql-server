...
func eraseProjection(ctx *sql.Context, a *Analyzer, node sql.Node, scope *Scope) (sql.Node, sql.TreeIdentity, error) {
	...
	return transform.Node(node, func(node sql.Node) (sql.Node, sql.TreeIdentity, error) {
		...
		if ok && project.Schema().Equals(project.Child.Schema()) {
			...
			return project.Child, sql.NewTree, nil
		}
...
func moveJoinConditionsToFilter(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope) (sql.Node, sql.TreeIdentity, error) {
	...
	node, same, err := transform.Node(n, func(n sql.Node) (sql.Node, sql.TreeIdentity, error) {
		...
		if filtersMoved == 0 {
			topJoin = n
			return topJoin, sql.SameTree, nil
		}
		...
		if len(condFilters) > 0 {
			...
			return topJoin, sql.NewTree, nil
		}
...
func evalFilter(ctx *sql.Context, a *Analyzer, node sql.Node, scope *Scope) (sql.Node, sql.TreeIdentity, error) {
	...
	return transform.Node(node, func(node sql.Node) (sql.Node, sql.TreeIdentity, error) {
		...
		e, same, err := transform.Exprs(filter.Expression, func(e sql.Expression) (sql.Expression, sql.TreeIdentity, error) {
			...
			case *expression.Or:
				if isTrue(e.Left) {
					return e.Left, sql.NewTree, nil
				}

				if isTrue(e.Right) {
					return e.Right, sql.NewTree, nil
				}
			...
			default:
				...
				return expression.NewLiteral(val, e.Type()), sql.NewTree, nil
			}
		})
		if err != nil {
			return nil, sql.SameTree, err
		...
		if isFalse(e) {
			return plan.EmptyTable, sql.NewTree, nil
		}

		if isTrue(e) {
			return filter.Child, sql.NewTree, nil
		}

		if same {
			return filter, sql.SameTree, nil
		}
		return plan.NewFilter(e, filter.Child), sql.NewTree, nil
	})
}
