...
func getJoinEqualityIndex(
	...
	leftIdx, rightIdx =
			ctx.IndexByExpression(ctx, a.Catalog.DefaultDatabase(), unifyExpressions(aliases, e.Left())...),
			ctx.IndexByExpression(ctx, a.Catalog.DefaultDatabase(), unifyExpressions(aliases, e.Right())...)
...
func getMultiColumnJoinIndex(ctx *sql.Context, exprs []sql.Expression, a *Analyzer, aliases map[string]sql.Expression, ) map[string]sql.Index {
	...
	for table, cols := range exprsByTable {
		idx := ctx.IndexByExpression(ctx, a.Catalog.DefaultDatabase(), unifyExpressions(aliases, extractExpressions(cols)...)...)
		if idx != nil {
			result[table] = idx
		}
	}
