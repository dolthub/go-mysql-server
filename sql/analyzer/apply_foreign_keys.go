...
func applyForeignKeysToNodes(ctx *sql.Context, a *Analyzer, n sql.Node, cache *foreignKeyTableCache) (sql.Node, error) {
	...
	case *plan.InsertInto:
		//TODO: perhaps use Children() and make a note in INSERT?
		...
		fkCheck := &plan.ForeignKeyReferenceChecker{
			Table:      tbl,
			Sch:        insertableDest.Schema(),
			References: make([]*plan.ForeignKeyReference, len(declaredFks)),
		}
		for i, declaredFk := range declaredFks {
			...
			fkCheck.References[i] = &plan.ForeignKeyReference{
				Index:          refIndex,
