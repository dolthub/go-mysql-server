...
func resolveDropConstraint(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope) (sql.Node, sql.TreeIdentity, error) {
	...
	if !ok {
		return nil, sql.SameTree, ErrInAnalysis.New("Expected a ResolvedTable for ALTER TABLE DROP CONSTRAINT statement")
	}
	...
	if ok {
		...
		for _, fk := range fks {
			if strings.ToLower(fk.Name) == strings.ToLower(dropConstraint.Name) {
				return plan.NewAlterDropForeignKey(rt, dropConstraint.Name), sql.NewTree, nil
			}
		}
	}
	...
	if ok {
		...
		for _, check := range checks {
			if strings.ToLower(check.Name) == strings.ToLower(dropConstraint.Name) {
				return plan.NewAlterDropCheck(rt, check.Name), sql.NewTree, nil
			}
		}
	}

	return nil, sql.SameTree, sql.ErrUnknownConstraint.New(dropConstraint.Name)
}

// validateDropConstraint returns an error if the constraint named to be dropped doesn't exist
func validateDropConstraint(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope) (sql.Node, sql.TreeIdentity, error) {
	switch n := n.(type) {
	case *plan.DropForeignKey:
		...
		if ok {
			...
			return nil, sql.SameTree, sql.ErrUnknownConstraint.New(n.Name)
		}

		return nil, sql.SameTree, plan.ErrNoForeignKeySupport.New(rt.Table.Name())
	case *plan.DropCheck:
		rt, ok := n.Child.(*plan.ResolvedTable)
		if !ok {
			return nil, sql.SameTree, ErrInAnalysis.New("Expected a ResolvedTable for ALTER TABLE DROP CONSTRAINT statement")
		...
		return nil, sql.SameTree, plan.ErrNoCheckConstraintSupport.New(rt.Table.Name())
	default:
		return n, sql.SameTree, nil
	}
}
