...
var validationRules2 = [14]func(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector){
	validateIsResolved2,
	validateOrderBy2,
	validateDeleteFrom2,
	validateGroupBy2,
	validateSchemaSource2,
	validateIndexCreation2,
	validateUnionSchemasMatch2,
	validateIntervalUsage2,
	validateStarExpressions2,
	validateOperands2,
	validateSubqueryColumns2,
	validateAggregations2,
}

var validationRulesBefore = [12]func(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector){
	validateLimitAndOffset2,
	validateDeleteFrom2,
	validatePrivileges,
	validateCreateTable,
	validateExprSem2,
	validateDropConstraint,
	validateReadOnlyDatabase2,
	validateReadOnlyTransaction2,
}

// validateAll runs all validation rules
func validateBefore(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) (sql.Node, transform.TreeIdentity, error) {
	var err error
	defer func() {
		if r := recover(); r != nil {
			switch e := r.(type) {
...
// validateAll runs all validation rules
func validateAfter(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) (sql.Node, transform.TreeIdentity, error) {
	var err error
	defer func() {
		if r := recover(); r != nil {
			switch e := r.(type) {
...
func validateLimitAndOffset2(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) {
	var err error
	var i, i64 interface{}
...
func validateIsResolved2(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) {
...
func unresolvedError2(n sql.Node) {
	var err error
...
func validateOrderBy2(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) {
...
func validateDeleteFrom2(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) {
	span, ctx := ctx.Span("validate_order_by")
	defer span.End()
...
func checkSqlMode2(ctx *sql.Context, option string) bool {
...
func validateGroupBy2(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) {
	span, ctx := ctx.Span("validate_group_by")
	defer span.End()

	// only enforce strict group by when this variable is set
	if isStrict := checkSqlMode2(ctx, "ONLY_FULL_GROUP_BY"); !isStrict {
		return
	}
...
func validateSchemaSource2(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) {
...
func validateIndexCreation2(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) {
...
func validateUnionSchemasMatch2(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) {
...
func validateIntervalUsage2(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) {
...
func validateStarExpressions2(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) {
			...
			for _, e := range er.Expressions() {
				...
				sql.Inspect(e, func(e sql.Expression) bool {
					...
					case *expression.Star:
						panic(semError{analyzererrors.ErrStarUnsupported.New()})
...
func validateOperands2(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) {
...
func validateSubqueryColumns2(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) {
...
func validateReadOnlyDatabase2(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) {
...
func validateReadOnlyTransaction2(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) {
...
func validateAggregations2(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) {
	transform.Inspect(n, func(n sql.Node) bool {
		switch n := n.(type) {
		case *plan.GroupBy:
			checkForAggregationFunctions2(n.GroupByExprs)
		case *plan.Window:
			checkForNonAggregatedColumnReferences2(n)
		case sql.Expressioner:
			checkForAggregationFunctions2(n.Expressions())
		default:
		}
		return false
	})
...
func checkForAggregationFunctions2(exprs []sql.Expression) {
	for _, e := range exprs {
...
func checkForNonAggregatedColumnReferences2(w *plan.Window) {
...
func validateExprSem2(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) {
	transform.InspectExpressions(n, func(e sql.Expression) bool {
		validateSem2(e)
		return false
	})
...
// Add node equivalent.
func validateSem2(e sql.Expression) error {
	switch e := e.(type) {
	case *expression.And:
