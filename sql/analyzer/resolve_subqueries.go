...
func setJoinScopeLen(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope, sel RuleSelector) (sql.Node, transform.TreeIdentity, error) {
	...
	return transform.Node(n, func(n sql.Node) (sql.Node, transform.TreeIdentity, error) {
		if j, ok := n.(*plan.JoinNode); ok {
			...
			hack := false
			if rightSubqueryAlias, ok := nj.Right().(*plan.SubqueryAlias); ok {
				hack = rightSubqueryAlias.OuterScopeVisibility
			}
			if _, ok := nj.Left().(*plan.StripRowNode); !ok {
				...
				if !hack {
					...
					// TODO: ResolveCoercibility then GetCoercibility
					rightScope := scope.newScope(nj.Left())
					rightScopeLen := len(rightScope.Schema())
					nnj, err = nj.WithChildren(
						plan.NewStripRowNode(nj.Left(), scopeLen),
						plan.NewStripRowNode(nj.Right(), rightScopeLen), // TODO: scope length should change if nj.Right can see left
					)
				}
