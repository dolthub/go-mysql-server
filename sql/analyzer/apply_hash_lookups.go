...
func applyHashLookups(ctx *sql.Context, a *Analyzer, n sql.Node, scope *Scope) (sql.Node, error) {
	return plan.TransformUpWithParent(n, func(n sql.Node, parent sql.Node, childNum int) (sql.Node, error) {
		...
		if isCachedResults && pij != nil && childNum == 1 {
			primary := pij.Left()
			cond = pij.Cond
		...
		} else if isCachedResults && pj != nil && pj.JoinType() != plan.JoinTypeRight && childNum == 1 {
			primary := pj.Left()
			cond = pj.JoinCond()
			primaryIndex := len(primary.Schema()) + len(scope.Schema())
			primaryGetter = getFieldIndexRange(0, primaryIndex, 0)
			secondaryGetter = getFieldIndexRange(primaryIndex, -1, primaryIndex)
		} else if isCachedResults && pj != nil && pj.JoinType() == plan.JoinTypeRight && childNum == 0 {
		...
		} else if j, ok := n.(plan.JoinNode); ok {
			// JoinNode implements a number of join modes,
			// some of which put all results from the
			// primary or secondary table in memory. This
			// hash lookup implementation is expecting
			// multipass mode, so we apply that here if we
			// have a JoinNode whose secondary child is a
			// HashLookup.
		...
		if cond != nil {
			...
			sql.Inspect(cond, func (e sql.Expression) bool {
				...
				case *expression.Equals:
					...
					} else if sgf := secondaryGetter(e.Left()); sgf != nil {
						if pgf := primaryGetter(e.Right()); pgf != nil {
							primaryGetFields = append(primaryGetFields, pgf)
							secondaryGetFields = append(secondaryGetFields, sgf)
						} else {
							validCondition = false
