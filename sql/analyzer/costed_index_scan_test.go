 ...
 └─ Eq
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := newIndexCoster(ctx, "xy")
			root, leftover, _ := c.buildRoot(tt.in, NewDefaultLogicTreeWalker(), make(map[string]string))
...
func TestRangeBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("Expr:  %s\nRange: %s", tt.filter.String(), tt.exp.DebugString()), func(t *testing.T) {
			...
			root, _, _ := c.buildRoot(tt.filter, NewDefaultLogicTreeWalker(), make(map[string]string))
...
func TestRangeBuilderInclude(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			root, _, _ := c.buildRoot(tt.in, NewDefaultLogicTreeWalker(), make(map[string]string))
