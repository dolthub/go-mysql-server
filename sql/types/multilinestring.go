...
func (t MultiLineStringType) Convert(v interface{}) (interface{}, bool, error) {
	switch buf := v.(type) {
	case nil:
	...
	case []byte:
		...
		if sql.ErrInvalidGISData.Is(err) {
			return nil, false, sql.ErrInvalidGISData.New("MultiLineString.Convert")
		}
		return mline, false, err
	case string:
